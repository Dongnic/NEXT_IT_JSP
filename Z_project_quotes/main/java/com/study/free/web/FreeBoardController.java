...
//		logger.info("searchVO : {}",searchVO);
	...
	public String freeView(Model model, @RequestParam(required = false) int boNo) {
		try{
			FreeBoardVO freeBoard=freeBoardService.getBoard(boNo); // attach도 같이 
	...
	public String freeModify(Model model ,@ModelAttribute("freeBoard") @Validated(value = {Modify.class, Default.class}) FreeBoardVO freeBoard, BindingResult error, @RequestParam(required = false) MultipartFile[] boFiles) throws IOException {
		...
		// regist처럼 multipartFile 파라미터는 파일업로드 후 freeBoard에 set
...
//		}
	...
	public String freeRegist(Model model, @ModelAttribute("freeBoard") @Validated() FreeBoardVO freeBoard, BindingResult error
		...
		ResultMessageVO resultMessageVO= new ResultMessageVO();
		System.out.println(boFiles.length);
		if(error.hasErrors()) {
			return "free/freeForm";
		}
		List<AttachVO> attaches = studyAttachUtils.getAttachListByMultiparts(boFiles, "FREE", "free");
		// "FREE"는 DB에 넣을 때 atchCategory, "free"는 /home/pc25/upload/free 폴더에 저장
